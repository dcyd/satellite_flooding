...
class SN8Dataset(Dataset):
    def __init__(self,
                 csv_filename: str,
                 data_to_load: List[str] = ["preimg","postimg","building","road","roadspeed","flood","flood_road","flood_building","pre_modis","post_modis"],
        ...
        """
        self.all_data_types = ["preimg","postimg","building","road","roadspeed","flood","flood_road","flood_building","pre_modis","post_modis"]
    ...
    def __getitem__(self, index):
        ...
        for i in self.all_data_types:
            ...
            if filepath is not None:
                ...
                # ds = self.get_warped_ds(data_dict[i])
                if self.img_size[0] == 1300:
                    ds = gdal.Open(filepath)
                else:
                    if i == "pre_modis" or i == "post_modis":
                        ds = self.get_warped_ds(data_dict[i],gdal.GDT_Float32)
                    else:
                        ds = self.get_warped_ds(data_dict[i],gdal.GDT_Byte)

                # if i == "postimg":
                #     ds = self.get_warped_ds(data_dict["postimg"],gdal.GDT_Byte)
                # else:
                #     ds = gdal.Open(filepath)
    ...
    def get_warped_ds(self, post_image_filename: str, data_type) -> gdal.Dataset:
        ...
        # dataset = gdal.Open(post_image_filename)
