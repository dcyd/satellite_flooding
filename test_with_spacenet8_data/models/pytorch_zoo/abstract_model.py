...
class AbstractModel(nn.Module):
    ...
    def initialize_encoder(self, model, model_url,num_channels):
        pretrained_dict = model_zoo.load_url(model_url)
        model_dict = model.state_dict()
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
        model.load_state_dict(pretrained_dict)
        if num_channels==4:
            model.initial_additional_layer()
...
class PseudoSiameseEncoderDecoder(AbstractModel):
    def __init__(self, num_classes, num_channels_1, num_channels_2, encoder_name='resnet34'):
        super().__init__()
        self.filters = encoder_params[encoder_name]['filters']
        ...
        # Define two encoders, not share weights
        encoder_1 = encoder_params[encoder_name]['init_op'](num_channels=num_channels_1)  # for vhr
        encoder_2 = encoder_params[encoder_name]['init_op'](num_channels=num_channels_2)  # for pre and post modis
        #print ("abstract_model.py, class EncoderDecoder, encoder:", encoder)
        #print ("abstract_model.py, class EncoderDecoder, num_channels:", num_channels)

        if num_channels_1 == 3 and encoder_params[encoder_name]['url'] is not None:
            self.initialize_encoder(encoder_1, encoder_params[encoder_name]['url'],num_channels=num_channels_1)
        else:
            print ("Couldn't initialize model for vhr...")

        if num_channels_2 == 4 and encoder_params[encoder_name]['url'] is not None:
            self.initialize_encoder(encoder_2, encoder_params[encoder_name]['url'],num_channels=num_channels_2)
        else:
            print ("Couldn't initialize model for modis...")

        self.encoder_stages_1 = nn.ModuleList([self.get_encoder(encoder_1, idx) for idx in range(len(self.filters))])
        self.encoder_stages_2 = nn.ModuleList([self.get_encoder(encoder_2, idx) for idx in range(len(self.filters))])
        # if num_channels_1 == 3:
        #    encoder = encoder_params[encoder_name+'_3channel']['init_op']()
        # else:
        #    encoder = encoder_params[encoder_name+'_5channel']['init_op']()
    ...
    def forward_once_1(self, x):
        # Encoder vhr
        enc_results = []
        for idx, stage in enumerate(self.encoder_stages_1):
            x = stage(x)
            if idx < len(self.encoder_stages_1) - 1:
                enc_results.append(x.clone())
    ...
    def forward_once_2(self, x):
        # Encoder pre post modis
        enc_results = []
        for idx, stage in enumerate(self.encoder_stages_2):
            x = stage(x)
            if idx < len(self.encoder_stages_2) - 1:
                enc_results.append(x.clone())
    ...
    def forward(self, x1, x2=None): # when n_classes is an int
        if isinstance(self.num_classes, int):
            out1 = self.forward_once_1(x1)
            out2 = self.forward_once_2(x2)
            x = torch.cat([out1, out2], dim=1)
            x = self.penultimate_conv(x)
            x = self.final(x)
            return x
