...
def create_mask_for_tile(pre_image,
    ...
    mask_types = ["flood_road", "flood_building"]
    ...
    if "flood_road" in mask_types:
        # now combine the building flood and road flood masks into a single flood mask
        # channel 1 has non-flooded roads
        # channel 2 has flooded roads
        flood_arr = np.zeros((2, preim_ncols, preim_nrows))
        flood_arr[0][road_flood_arr==1] = 1
        flood_arr[1][road_flood_arr==2] = 1

        # flood mask tile
        out_imagename = f"flood_road_{os.path.basename(pre_image).split('.')[0]}.tif"
        output_tif = os.path.join(output_image_directory, out_imagename)
        write_geotiff(output_tif, preim_ncols, preim_nrows,
                      xmin, xres, ymax, yres,
                     raster_srs, flood_arr)

    if "flood_building" in mask_types:
        # now combine the building flood and road flood masks into a single flood mask
        # channel 1 has non-flooded buildings
        # channel 2 has flooded buildings
        ...
        # flood mask tile
        out_imagename = f"flood_building_{os.path.basename(pre_image).split('.')[0]}.tif"
        output_tif = os.path.join(output_image_directory, out_imagename)
        write_geotiff(output_tif, preim_ncols, preim_nrows,
                      xmin, xres, ymax, yres,
                     raster_srs, flood_arr)
...
if __name__ == "__main__":
    # 	python -u "e:\code\Py_workplace\spacenet8\baseline\data_prep\create_masks.py" 
    # --root_dir "E:\code\Py_workplace\spacenet8\Data" 
