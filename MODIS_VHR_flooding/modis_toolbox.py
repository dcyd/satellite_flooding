...
def join_collections(img_coll1, img_coll2):
    """join_collections joins the GQ & GA products base on similar timefields.  
    The bands of the each image are concatenated together.
    The results is several bands with different resolutions!
    """
...
def pan_sharpen(image):
    ...
    Pay careful attention to the band names in the function.  These need to be present in the image passed to the function
...
def get_modis_terra_aqua(roi_bounds, date_range):

    def region_clip(img):
        return img.clip(roi_bounds)
    
    terra = get_terra(roi_bounds, date_range).map(region_clip)
    aqua = get_aqua(roi_bounds, date_range).map(region_clip)
